...
const mapTwoToThe = (arr) => {
  let squares = arr.map(num => Math.pow(2, num));
  return squares;
};
...
const charCode = (arr) => {
  let char = arr.map(code => code.charCodeAt());
  return char;
};
...
const extractAbilities = (arr) => {
  let abilities = arr.map(obj => obj.ability.name);
  return abilities;
};
